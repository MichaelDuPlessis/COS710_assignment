...
# programs are cloned in crossover
def crossover(program1: Node, program2: Node, max_depth: int, gsim: Set[List[int]], structure_depth: int) -> Tuple[Node, Node]:
    while True:
        program1, program2 = copy.deepcopy(program1), copy.deepcopy(program2)
        ...
        if program1.to_structure_list(structure_depth) not in gsim and program2.to_structure_list(structure_depth) not in gsim:
            return program1, program2

# programs are cloned in mutate
# max depth is used when generating subtrees
def mutate(program: Node, max_depth: int, gsim: Set[List[Node]], structure_depth: int) -> Node:
    from population_module.generation import generate_tree
